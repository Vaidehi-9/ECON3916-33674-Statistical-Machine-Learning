...
1. **The Failed Population**: Complete records of startups that received initial funding but never achieved unicorn status—including:
...
2. **The Selection Indicator**: A binary variable encoding whether each startup *would have been covered by TechCrunch if it had succeeded*. This requires:
...
3. **The Exclusion Restriction**: At least one variable that affects *probability of observation* (media coverage) but not *probability of success* (unicorn status). Example candidates:
...
**Course**: Tech Economics  
**Focus**: Data Generating Process, Sampling Methodology, Bias Detection  
**Date**: February 2026
